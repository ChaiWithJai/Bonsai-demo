...
<main>
  ...
  {#if model}
    <header><p class="eyebrow">BONSAI / YOUR DATA</p><h1>{model.plan.title}</h1><p>{model.plan.summary}</p><p class="provenance">{model.rows.length} source records · Semiotic {model.chart.component} · {model.excluded_record_ids.length} records without plotted coordinates</p></header>
