<script>
  ...
  let notes = $state([]);
  const discussionNotes = $derived(notes.filter(saved=>saved.review_origin !== 'workspace-automated-check'));
  const verificationNotes = $derived(notes.filter(saved=>saved.review_origin === 'workspace-automated-check'));
  let error = $state('');
...
<main>
  ...
  {#if model}
    ...
    <!-- svelte-ignore a11y_no_noninteractive_tabindex -->
    <div class="chart-viewport" role="region" aria-label="Scrollable chart" tabindex="0"><section class="chart" aria-label="Data visualization" style:max-width={chartScale ? 'none' : model.chart.props.width+'px'} style:width={chartScale ? (model.chart.props.width*chartScale)+'px' : '100%'}><img src="/api/chart.svg" alt={model.plan.title}/>{#each model.interaction?.points ?? [] as point (point.record_id)}{@const row=model.rows.find(row=>row.id===point.record_id)}{#if row}<button class="point-target" class:chosen={selected?.id===row.id} style:left={(point.x / model.interaction.width * 100)+'%'} style:top={(point.y / model.interaction.height * 100)+'%'} aria-label={'Inspect observation '+Object.values(row.data).join(', ')} aria-pressed={selected?.id===row.id} title={Object.values(row.data).join(' · ')} onclick={()=>selectRecord(row)}></button>{/if}{/each}{#each model.interaction?.nodes ?? [] as node (node.id)}<button class="node-target" class:chosen={group === node.id} style:left={(node.x / model.interaction.width * 100) + '%'} style:top={(node.y / model.interaction.height * 100) + '%'} aria-label={'Explore '+node.label} title={node.label} onclick={()=>group=node.id}></button>{/each}</section></div>
    ...
    <div class="panes"><section aria-label="Source records"><h2>Records <small>{visible.length}</small></h2>{#each visible as row (row.id)}<article data-testid="record-row" data-record-id={row.id} class:selected={selected?.id === row.id}>
      ...
      <details class="raw-data"><summary>Raw data</summary><pre data-testid="record-data">{JSON.stringify(row.data,null,2)}</pre></details>{#if row.locator.source_evidence}<details><summary>Supporting source passages</summary>{#each sourceGroups(row) as source (source.key)}<section class="source-citation" aria-label={'Evidence from '+source.label}><h4>{source.label}</h4>{#each source.passages as passage,i (i)}<p class="provenance">{passage.field.startsWith('locator.') ? 'Source position · '+fieldLabel(passage.field.slice(8)) : 'Source field · '+fieldLabel(passage.field)}</p><blockquote>{passage.quote}</blockquote>{/each}{#if source.link}<a href={source.link.url} target="_blank" rel="noreferrer">{source.link.label}</a>{/if}</section>{/each}</details>{/if}</article>{/each}</section>
    <aside>{#if selectedMedia.length}<section class="source-media" aria-label="Selected source media"><h2>Check the original</h2>{#each selectedMedia as media (media.key)}<article><h3>{media.label} · {media.seconds.toFixed(1)}s</h3>{#if media.kind==='video'}<!-- Original source; no caption track is created by extraction. --><!-- svelte-ignore a11y_media_has_caption --><video controls playsinline preload="metadata" src={media.url} onloadedmetadata={event=>{event.currentTarget.currentTime=media.seconds;}}></video>{:else}<audio controls preload="metadata" src={media.url} onloadedmetadata={event=>{event.currentTarget.currentTime=media.seconds;}}></audio>{/if}<a href={media.url} target="_blank" rel="noreferrer">Open original source</a></article>{/each}<p class="provenance">The player shows the original file at the cited time. Extracted observations and identities still need review.</p></section>{/if}<h2>Evidence notes</h2>{#if selected}<p class="selected-context">Adding a note to <strong>{recordTitle(selected)}</strong></p><form onsubmit={event=>{event.preventDefault();save();}}><label>Evidence note<textarea bind:value={note} required maxlength="4000"></textarea></label><button disabled={!note.trim() || savingNote}>{savingNote ? 'Saving note…' : 'Save note'}</button></form>{:else}<p>Select a record to attach a note.</p>{/if}{#each discussionNotes as saved (saved.id)}<article><p>{saved.note}</p><small>{recordTitle(saved.record_snapshot)} · {saved.review_origin}</small></article>{/each}{#if verificationNotes.length}<details class="verification-notes"><summary>Automated verification notes ({verificationNotes.length})</summary><p class="provenance">Saved by browser checks to verify note persistence. These are not team reviews.</p>{#each verificationNotes as saved (saved.id)}<article><p>{saved.note}</p><small>{recordTitle(saved.record_snapshot)} · {saved.review_origin}</small></article>{/each}</details>{/if}</aside></div>
  {:else}<p role="status">Loading source records…</p>{/if}
</main>
<style>
  .point-target{position:absolute;width:24px;height:24px;padding:0;border:0;border-radius:50%;background:transparent;transform:translate(-50%,-50%)}.point-target::after{content:"";position:absolute;inset:8px;background:#fff;border:2px solid #506a51;border-radius:50%}.point-target:hover::after,.point-target.chosen::after{background:#506a51}.point-target:focus-visible{outline:2px solid #506a51}
...
.source-citation{margin:12px 0;padding:12px;border:1px solid #dce2d6;border-radius:8px}.source-citation h4{margin:0 0 12px;font-size:13px;overflow-wrap:anywhere}.source-citation blockquote{white-space:pre-wrap;overflow-wrap:anywhere}.source-citation .provenance{margin-bottom:4px}
</style>
