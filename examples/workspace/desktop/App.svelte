...
<main>
  ...
  {#if model}
    ...
    {#if model.chart.component === 'RecordTable'}
      ...
      <div class="table-scroll" tabindex="0" role="region" aria-label="Source record table"><table><thead><tr>{#each model.chart.props.columns as column (column)}<th scope="col">{column}</th>{/each}<th scope="col">Evidence</th></tr></thead><tbody>{#each visible as row (row.id)}<tr data-table-record-id={row.id}>{#each model.chart.props.columns as column (column)}<td>{row.data[column] == null ? 'Missing' : String(row.data[column])}</td>{/each}<td><button onclick={()=>selected=row} aria-label={'Review table record '+row.id}>Review</button></td></tr>{/each}</tbody></table></div>
    {:else}
...
<style>
  .table-scroll{max-width:100%;overflow-x:auto;border:1px solid #d9ddd6;border-radius:10px}table{width:100%;border-collapse:collapse;background:white}th,td{text-align:left;padding:12px;border-bottom:1px solid #d9ddd6;vertical-align:top;overflow-wrap:anywhere}th{font-size:12px}
