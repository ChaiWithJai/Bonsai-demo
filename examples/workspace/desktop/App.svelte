...
<main>
  ...
  {#if model}
    ...
    <div class="panes"><section aria-label="Source records"><h2>Records <small>{visible.length}</small></h2>{#each visible as row (row.id)}<article data-testid="record-row" data-record-id={row.id}><button onclick={()=>selected=row} aria-label={'Inspect '+row.id}>{String(row.data.model ?? row.data.title ?? locationLabel(row.locator))}</button><pre data-testid="record-data">{JSON.stringify(row.data,null,2)}</pre>{#if row.locator.source_evidence}<details><summary>Supporting source passages</summary>{#each row.locator.source_evidence as passage,i (i)}<blockquote>{passage.quote}</blockquote>{#if model.evidence_links?.[passage.record_id]}<a href={model.evidence_links[passage.record_id].url} target="_blank" rel="noreferrer">{model.evidence_links[passage.record_id].label}</a>{/if}{/each}</details>{/if}</article>{/each}</section>
