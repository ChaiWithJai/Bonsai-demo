...
<main>
  ...
  {#if model}
    ...
    <aside><h2>Evidence notes</h2>{#if selected}<p class="selected-context">Adding a note to <strong>{String(selected.data.title ?? selected.data.model ?? locationLabel(selected.locator))}</strong></p><form onsubmit={event=>{event.preventDefault();save();}}><label>Evidence note<textarea bind:value={note} required maxlength="4000"></textarea></label><button disabled={!note.trim()}>Save note</button></form>{:else}<p>Select a record to attach a note.</p>{/if}{#each notes as saved (saved.id)}<article><p>{saved.note}</p><small>{locationLabel(saved.record_snapshot.locator)} · {saved.review_origin}</small></article>{/each}</aside></div>
