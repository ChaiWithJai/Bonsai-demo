<script lang="ts">
  ...
  let { source, onSaved }: {source: SourceData; onSaved: () => Promise<void>} = $props();
  let recordId = $state('');
  let author = $state('');
  ...
  function chooseRecord(event: Event & {currentTarget: HTMLSelectElement}) {
    ...
    correction = row ? JSON.stringify(row.data, null, 2) : '';
  ...
  async function saveReview() {
    ...
    try {
      await request('/api/workspace/sources/' + source.source_id + '/review', {source_id:source.source_id,snapshot_id:source.review.snapshot_id,
        ...
        action,note,corrected_data:action === 'correct' ? JSON.parse(correction) : null});
      await onSaved();
...
<details class="record-review">
  ...
  <select id="review-record" value={recordId} onchange={chooseRecord}>
    ...
    {#each source.records as row, index (row.id)}<option value={row.id}>Record {index + 1} · {JSON.stringify(row.locator)}</option>{/each}
  </select>
  ...
  {#if record}
    ...
    {#if action === 'correct'}<label for="review-correction">Corrected record JSON</label><textarea id="review-correction" bind:value={correction} rows="8" spellcheck="false"></textarea>{/if}
