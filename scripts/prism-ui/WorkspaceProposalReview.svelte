<script lang="ts">
  ...
  let published = $state<{example_count:number;run_url:string} | null>(null);
  ...
  async function publish() {
    busy=true;error='';published=null;
    try {published=await api('/review-export',{});}
    catch(e){error='MLflow publication failed. Your reviews remain saved; you can download the bundle or retry. '+String(e);}
    finally{busy=false;}
  }
...
<details class="proposal-review" ontoggle={event=>{if(event.currentTarget.open && !review && !busy)void load();}}>
  ...
  {#if review}
    ...
    <button class="publish" disabled={busy} onclick={publish}>Publish example bundle to MLflow</button>
    {#if published}<p role="status">Published {published.example_count} eligible examples. <a href={published.run_url} target="_blank" rel="noreferrer">Open dataset run</a></p>{/if}
