<script lang="ts">
  import WorkspaceViewChoice from '$lib/WorkspaceViewChoice.svelte';
  import WorkspaceChartPreview from '$lib/WorkspaceChartPreview.svelte';
  import WorkspacePdfPage from '$lib/WorkspacePdfPage.svelte';
  import {onMount} from 'svelte';
  type Proposal = {source_review?:{reason:string;evidence:{record_id:string;field:string;quote:string}}[];structure?: {rationale:string;records:{values:Record<string,unknown>;evidence:{record_id:string;field:string;quote:string}[]}[]} | null;interpretation:{findings:{text:string;record_ids:string[]}[];rationale:string;uncertainties:string[];questions:string[]};plan:{title:string;summary:string;fields:{name:string;type:string}[];view:{component:string;groupBy?:string[];x?:string;y?:string;color?:string}}};
  let {proposal, previewJobId = '', viewSelected = false, retentionCount = 0, sourceFilename = '', draftKey = '', coverage, evidence = [], busy = false, readOnly = false, onSelectView, onConfirm, onRevise} = $props<{proposal:Proposal;previewJobId?:string;viewSelected?:boolean;retentionCount?:number;sourceFilename?:string;draftKey?:string;coverage?:{structured_usage?:{supplied_records:number;cited_records:number;uncited_record_ids:string[];scope:string} | null;source_scope?:{pages:number[];records_in_scope:number;records_outside_scope:number};records_shown:number;records_total:number;coverage:string;requested_page_coverage?:{requested:number[];shown:number[];omitted:number[];not_found:number[]};member_coverage?:{source_id:string;filename:string;records_shown:number;records_total?:number;represented:boolean}[]};evidence?:{id:string;locator:Record<string,unknown>;data:Record<string,unknown>}[];busy?:boolean;readOnly?:boolean;onSelectView?:(view:Record<string,unknown>)=>void;onConfirm:()=>void;onRevise:(feedback:string,reviewUnusedSources?:boolean)=>void}>();
  let feedback=$state('');
  let reviewUnusedSources=$state(false);
  let draftReady=$state(false);
  let draftNotice=$state('');
  onMount(()=>{
    if(draftKey && !readOnly) {
      try {feedback=localStorage.getItem('bonsai-proposal-feedback:v1:'+draftKey)?.slice(0,4000) ?? '';reviewUnusedSources=localStorage.getItem('bonsai-proposal-source-review:v1:'+draftKey)==='true';}
  ...
  $effect(()=>{
    ...
    try {
      ...
      localStorage.setItem('bonsai-proposal-source-review:v1:'+draftKey,String(reviewUnusedSources));
...
<section class="proposal" aria-label="Bonsai proposal">
  ...
  {/if}
  {#if proposal.source_review?.length}<section class="file-coverage" aria-label="Proposed source exclusions"><h4>Sources Bonsai proposes to leave out</h4><p class="coverage">These explanations are model judgments, not verified exclusions. Review them against your question.</p>{#each proposal.source_review as entry,i (i)}{@const link=sourceLink(entry.evidence.record_id)}<article><p>{entry.reason}</p><blockquote>{entry.evidence.quote}</blockquote><p class="coverage">Source field: {entry.evidence.field}</p>{#if link}<a href={link.url} target="_blank" rel="noreferrer">{link.label}</a>{/if}</article>{/each}</section>{/if}
  ...
  <h3>{viewSelected ? 'Selected view' : 'I suggest'}: {proposal.plan.title}</h3><p>{viewSelected ? proposal.plan.summary : proposal.interpretation.rationale}</p>
  ...
  {#if previewJobId && proposal.plan.view.component!=='RecordTable'}{#key previewJobId}<WorkspaceChartPreview jobId={previewJobId} title={proposal.plan.title}/>{/key}{/if}
  {#if !readOnly && onSelectView}<WorkspaceViewChoice fields={proposal.plan.fields} busy={busy || Boolean(feedback.trim())} onSelect={onSelectView}/>{/if}
  ...
  {#if proposal.interpretation.uncertainties.length}<h4>{viewSelected ? 'Uncertainties from the earlier interpretation' : 'What remains uncertain'}</h4><ul>{#each proposal.interpretation.uncertainties as item (item)}<li>{item}</li>{/each}</ul>{/if}
  ...
  {#if coverage?.structured_usage?.uncited_record_ids.length}<label class="source-review-toggle"><input type="checkbox" bind:checked={reviewUnusedSources} disabled={busy || !coverage?.structured_usage?.uncited_record_ids.length}/> Require an observation or a quoted exclusion explanation for each unused source</label>{/if}
  <div class="actions"><button onclick={()=>onRevise(feedback,reviewUnusedSources)} disabled={busy || !feedback.trim()}>Discuss this change</button><button class="confirm" onclick={onConfirm} disabled={busy || Boolean(feedback.trim())}>Yes, build this view</button></div>
