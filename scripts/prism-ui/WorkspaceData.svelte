...
<section class="data" aria-label="Desktop sources">
  ...
  <label class="upload">{busy ? 'Extracting source; PDFs may take a moment…' : 'Attach files'}<input type="file" multiple accept=".xlsx,.docx,.eml,.mbox,.csv,.tsv,.json,.jsonl,.txt,.md,.png,.jpg,.jpeg,.webp,.pdf,.wav,.mp3,.m4a,.mp4,.mov,.webm" onchange={upload} disabled={busy}/></label>
