...
<section class="data" aria-label="Desktop sources">
  ...
  <p class="fine">Up to 20 files, 25 MiB each. Tables, text, email exports, and PDF pages are extracted locally. PDF pages without embedded text use OCR on this Mac. Images use local OCR. English audio uses local Whisper transcription. Bonsai can read image content and sampled video frames for review.</p>
  ...
  {#if source}
    ...
    {#if source.status === 'extracted'}
      ...
      {#if source.kind === 'audio'}<section aria-label="Audio evidence"><h3>Listen and check the transcript</h3><audio controls bind:this={audioPlayer} src={'/api/workspace/sources/'+source.source_id+'/file'}></audio><div class="transcript">{#each source.records as row (row.id)}<button aria-pressed={selectedRecord===row.id} onclick={()=>{selectedRecord=row.id;if(audioPlayer){audioPlayer.currentTime=Number(row.locator.start_seconds);void audioPlayer.play().catch(()=>{});}}}>{Number(row.locator.start_seconds).toFixed(1)}s · {String(row.data.text)}</button>{/each}</div></section>{:else if source.kind === 'video'}<section aria-label="Video evidence"><h3>Check the sampled frames</h3><p>These records describe sampled frames. Select a record to inspect that moment in the original video.</p><!-- svelte-ignore a11y_media_has_caption (Source video is unmodified; this workflow extracts visual samples, not a caption track.) --><video controls muted playsinline bind:this={videoPlayer} src={'/api/workspace/sources/'+source.source_id+'/file'}></video><div class="transcript">{#each source.records as row (row.id)}<button aria-pressed={selectedRecord===row.id} onclick={()=>{selectedRecord=row.id;if(videoPlayer)videoPlayer.currentTime=Number(row.locator.time_seconds);}}>{Number(row.locator.time_seconds).toFixed(1)}s · {Object.entries(row.data).map(([key,value])=>key+': '+String(value)).join(' · ')}</button>{/each}</div></section>{:else if source.kind === 'image' && source.image_coverage}<WorkspaceImageEvidence {source} bind:selected={selectedRecord}/>{:else}<div class="records">{#each source.records.slice(0, 10) as row (row.id)}<details><summary>{JSON.stringify(row.locator)}</summary><pre>{JSON.stringify(row.data, null, 2)}</pre></details>{/each}</div>{/if}
