<script lang="ts">
  ...
  let { onProject = (_id: string) => {}, initialSource = '' } = $props<{onProject?: (id: string) => void; initialSource?:string}>();
...
<section class="data" class:empty={!source} aria-label="Desktop sources">
  ...
  <p class="welcome-message">Send me your files and a question. I’ll help organize the information and suggest a view we can refine together.</p>
  <div class="message-compose"><label for="visualization-intent">Message Bonsai</label><textarea id="visualization-intent" form="source-request" bind:value={intent} minlength="10" maxlength="4000" required placeholder="For example: help me explore the themes in these documents…" disabled={busy}></textarea>
  <label class="upload">{busy ? 'Extracting source; PDFs may take a moment…' : 'Attach files'}<input type="file" multiple accept=".xlsx,.docx,.eml,.mbox,.csv,.tsv,.json,.jsonl,.txt,.md,.png,.jpg,.jpeg,.webp,.pdf,.wav,.mp3,.m4a,.mp4,.mov,.webm" onchange={upload} disabled={busy}/></label></div>
  ...
  {#if sources.length}
    <details class="saved-source"><summary>Use a previously attached file</summary><label>Saved source<select value={source?.source_id ?? ''} onchange={event => choose(event.currentTarget.value)}><option value="" disabled>Choose a source</option>{#each sources as item (item.source_id)}<option value={item.source_id}>{item.filename} · {item.record_count} records</option>{/each}</select></label></details>
  {/if}
  {#if source}
    ...
    {#if ['image','video'].includes(source.kind)}<button onclick={readVision} disabled={busy || Boolean(runningJob)}>Read visual content with Bonsai</button><p class="fine">Extract visible tables and text. Video samples one frame every 15 seconds; audio and unsampled motion are not read.</p>{/if}
