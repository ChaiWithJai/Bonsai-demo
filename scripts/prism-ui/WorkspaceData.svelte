<script lang="ts">
  ...
  type Job = {id:string; source_id:string; filename:string; request:string; status:string; stage:string; workspace_id?:string; error?:string; mlflow_url?:string; proposal?:any; proposal_sha256?:string; source_coverage?:any; source_examples?:any[]};
  let jobs = $state<Job[]>([]);
  let intent = $state('');
