<script lang="ts">
  ...
  async function choose(id: string, offset = 0) {
    error = '';
    try { source = await request('/' + id + (offset ? '?offset='+offset : '')); selectedRecord='';
  ...
  async function refreshCurrentSource() {
    if(source) source=await request('/'+source.source_id+'?offset='+(source.record_offset ?? 0));
  }
...
<section class="data" class:empty={!source} class:starting={panel==='conversation' && !intakeHistory.length && !conversationJobs.length} aria-label="Desktop sources">
  ...
  {#if source}
    ...
    {#if source.status === 'extracted'}
      <p class="fine">Showing records {(source.record_offset ?? 0)+1}–{(source.record_offset ?? 0)+source.records.length} of {source.record_count}. Source IDs and locations are retained. Review the source evidence here or ask Bonsai to propose a view.</p>
      {#if (source.record_offset ?? 0)>0 || source.next_record_offset != null}<nav aria-label="Source record pages"><button onclick={()=>source && choose(source.source_id,Math.max(0,(source.record_offset ?? 0)-100))} disabled={busy || !(source.record_offset ?? 0)}>Previous records</button><button onclick={()=>source && source.next_record_offset != null && choose(source.source_id,source.next_record_offset)} disabled={busy || source.next_record_offset == null}>Next records</button></nav>{/if}
