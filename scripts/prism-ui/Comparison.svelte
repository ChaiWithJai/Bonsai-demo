<script lang="ts">
  ...
  let minP = $state(0);
  let seed = $state(42);
  let repeatPenalty = $state(1);
  const supportsThinking = $derived(status?.capabilities?.thinking_budget === true);
  ...
  function restore(saved: Data) {
    ...
    tokenLimit = saved.settings?.max_tokens ?? 4096;
    topP = saved.settings?.top_p ?? 0.95; topK = saved.settings?.top_k ?? 20;
    minP = saved.settings?.min_p ?? 0; seed = saved.settings?.seed ?? 42; repeatPenalty = saved.settings?.repeat_penalty ?? 1;
  ...
  function newComparison() { projectContext = ''; prompt = ''; run = null; outputs = {}; browserViews = {}; pausedViews = {}; error = ''; mode = 'sequential'; temperature = 0.3; tokenLimit = 4096; topP = 0.95; topK = 20; minP = 0; seed = 42; repeatPenalty = 1; thinking = false; cancellationRequested = false; }
  const effectiveTokenLimit = $derived(Math.min(tokenLimit, status?.limits?.max_tokens ?? 4096));
  ...
  async function start() {
    ...
    try {
      const response = await fetch('/api/comparison/run', {method: 'POST', signal: requestController.signal, headers: {'Content-Type': 'application/json'}, body: JSON.stringify({task: 'text', top_p: topP, top_k: topK, min_p: minP, seed, repeat_penalty: repeatPenalty, project_context: projectContext.trim(), prompt: prompt.trim(), mode: status?.modes?.includes(mode) ? mode : 'sequential', temperature, max_tokens: effectiveTokenLimit, thinking_budget_tokens: supportsThinking && thinking ? Math.min(status?.limits?.thinking_budget_tokens ?? 128, thinkingLimit) : 0})});
...
<div class="comparison-page">
  <header><div><p class="eyebrow">PRISM ML / MODEL COMPARISON</p><h1>One workspace. Two perspectives.</h1><p class="intro">Give both models the same prompt and context. Read their live responses side by side, then inspect the recorded evidence.</p></div><button class="button" onclick={() => refresh()} disabled={loading || running}><RefreshCw size={15}/>{loading ? 'Checking…' : 'Refresh status'}</button></header>
  {#if error}<div class="notice" role="alert"><CircleAlert size={18}/><span>{error}</span></div>{/if}
  {#if status?.recent_runs?.length}<div class="recent-runs"><label for="comparison-history">Load a saved run</label><select id="comparison-history" value={run?.run_id || ''} disabled={running} onchange={(event) => restoreId(event.currentTarget.value)}><option value="" disabled>Select a recorded run</option>{#each status.recent_runs as saved}<option value={saved.run_id}>{saved.status} · {saved.mode} · {(saved.settings?.prompt || saved.run_id).slice(0,80)}</option>{/each}</select><button class="button" onclick={newComparison} disabled={running}>New comparison</button><span class="caption">Load original outputs and settings. New runs use the shared text workspace.</span></div>{/if}
  <section class="prompt-card" aria-label="Shared comparison prompt">
    <div class="section-title"><span class="eyebrow">SHARED INPUT</span><span class="pill">Same prompt · no browser tools</span></div>
    <label class="prompt-label" for="project-context">Shared context <span class="caption">optional</span></label>
    <textarea id="project-context" bind:value={projectContext} disabled={running} rows="3" placeholder="Background, source material, constraints, or the audience for this response…"></textarea>
    <label class="prompt-label" for="comparison-prompt">Your prompt</label>
    ...
    <div class="controls"><fieldset disabled={running}><legend>Execution mode</legend><label><input type="radio" name="comparison-mode" bind:group={mode} value="sequential"/> Sequential</label>{#if status?.modes?.includes('concurrent')}<label><input type="radio" name="comparison-mode" bind:group={mode} value="concurrent"/> Concurrent · shared hardware</label>{/if}</fieldset>{#if supportsThinking}<label class="thinking"><input type="checkbox" bind:checked={thinking} disabled={running}/> Enable bounded reasoning ({Math.min(status?.limits?.thinking_budget_tokens ?? 128,thinkingLimit)} tokens)</label>{/if}</div>
    ...
    <fieldset class="settings-grid" disabled={running}><legend>Shared generation settings</legend>
      <label>Temperature<input type="number" min="0" max="2" step="0.05" bind:value={temperature}/></label>
      <label>Maximum tokens<input type="number" min="1" max={status?.limits?.max_tokens ?? 4096} step="1" bind:value={tokenLimit}/></label>
      <label>Top P<input type="number" min="0.000001" max="1" step="0.01" bind:value={topP}/></label>
      <label>Top K<input type="number" min="0" max="1000" step="1" bind:value={topK}/></label>
      <label>Min P<input type="number" min="0" max="1" step="0.01" bind:value={minP}/></label>
      <label>Seed<input type="number" min="0" max="2147483647" step="1" bind:value={seed}/></label>
      <label>Repeat penalty<input type="number" min="0.1" max="2" step="0.05" bind:value={repeatPenalty}/></label>
    </fieldset>
    <p class="caption">A fixed seed records the requested sampling seed. Matching settings do not guarantee identical sampling behavior across runtimes; inspect each recorded request.</p>
    <div class="run-controls"><span class="caption">Up to {effectiveTokenLimit} generated tokens per model · Shared input and settings</span><button class="button primary" onclick={start} disabled={!ready || running || cancellationRequested || !prompt.trim()}><Play size={15}/>{running ? 'Models running…' : 'Run both models'}</button>{#if running}<button class="button" onclick={cancel} disabled={cancellationRequested}>{cancellationRequested ? 'Cancellation requested…' : 'Cancel comparison'}</button>{/if}</div>
  ...
  <div class="model-grid">
    {#each status?.models || [] as model (model.id)}
      ...
      <section class="model-card" aria-label={model.label || model.id}>
        ...
        <p class="caption">Recorded weight format: <strong>{result?.identity?.quantization || result?.identity?.weight_dtype || model.quantization || model.identity?.weight_dtype || model.identity?.quantization || 'Not reported'}</strong> · Model provenance is available below.</p>
        ...
        <div class="metrics"><div><Timer size={14}/><small>Exchange elapsed</small><strong>{seconds(result?.elapsed_ms)}</strong></div><div><Activity size={14}/><small>First output delta</small><strong>{seconds(result?.time_to_first_token_ms)}</strong></div><div><Cpu size={14}/><small>{timings.scope === 'instrumented_prefill_and_decode' ? 'Output / total time' : 'Generation rate'}</small><strong>{metric(timings.predicted_per_second,' tok/s')}</strong></div></div>
        <p class="caption">First output delta is client-observed and includes emitted reasoning when enabled; it is not physical token timing.</p>{#if timings.scope === 'instrumented_prefill_and_decode'}<p class="caption">Output / total time includes prefill, decode, and activation capture. It is not decode-only or production throughput.</p>{/if}
        {#if result?.task === 'grant_research'}
        ...
        {#if result?.content}<details class="evidence"><summary>{typeof result?.content === 'string' && /<svg\b/i.test(result.content) ? 'Original SVG / model output' : 'Raw output'}</summary><pre>{result.content}</pre></details>{/if}
      </section>
    {/each}
  </div>
  {#if loading && !status}<div class="empty" role="status">Checking model endpoints…</div>{/if}
  <div class="notice"><Activity size={18}/><span>Qwen activations are captured during original inference; Bonsai activations are recorded in a separate replay. <a href="#/observability">Inspect the matching request, capture progress, and measured activations in Observability</a>. A replay is a separate execution, not the original live activations.</span></div>
...
<style>
  ...
  .settings-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(115px,1fr));gap:14px;margin-top:22px}.settings-grid label{display:flex;flex-direction:column;align-items:stretch;gap:7px;font-size:11px}.settings-grid input{width:100%;padding:10px;border:1px solid var(--border);border-radius:9px;background:var(--background);font:inherit}.settings-grid legend{margin-bottom:12px}
