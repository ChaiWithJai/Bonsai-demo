<script lang="ts">
  ...
  onMount(() => {
    ...
    async function init() {
      try {
        await refresh();
        const saved = localStorage.getItem('bonsai-workspace');
        const id = status?.workspaces?.find((p: Data) => p.id === saved)?.id ?? status?.workspaces?.[0]?.id;
...
<main class="workspace">
  <header class="heading">
    ...
    <div class="header-actions"><span class="model"><span class:active={running} class="dot"></span>Bonsai 2 · local</span><button onclick={create} disabled={busy || running || loading}><Plus size={15}/> New workspace</button></div>
  </header>
  {#if error}<p class="error" role="alert">{error}</p>{/if}
  {#if loading}<p role="status">Opening Workspace…</p>
  {:else if !project}
    <section class="empty"><Layers size={36}/><h2>Start with a working explorer.</h2><p>Open the cached Bonsai snapshot with 14 source records and saved evidence notes. Ask Bonsai to extend it, then inspect the change and its trace.</p><button class="primary" onclick={create} disabled={busy || !status}>Open source explorer <ArrowUp size={16}/></button><small>Cached observations · authored starter · no inference until you send a request</small></section>
  {:else}
    <div class="project-bar"><label>Project <select value={project.id} onchange={(event) => choose(event.currentTarget.value).catch(e => error = String(e))} disabled={running || busy}>{#each status?.workspaces ?? [] as item (item.id)}<option value={item.id}>{item.title}</option>{/each}</select></label><span>14 source records</span><span class="revision">Revision {project.head.slice(0, 10)}</span></div>
    <div class="panes">
      <section class="conversation" aria-label="Workspace conversation">
        <div class="conversation-body">
          <div class="context-card"><p class="eyebrow">STARTING POINT</p><h2>Bonsai source explorer</h2><p>Parameter-size groups, source links, and persistent notes. Extend the project without losing earlier work.</p><span>Cached observations · September 18, 2026</span></div>
