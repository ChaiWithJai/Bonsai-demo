...
def main():
    ...
    for folder in folders:
        audio=folder/'narration.mp3';suffix=('-'+args.language_hint+'-hint') if args.language_hint else '';target=folder/('transcript'+suffix+'.json');audio_hash=sha(audio)
        ...
        segments,info=model.transcribe(str(audio),language=args.language_hint,beam_size=5,word_timestamps=True,condition_on_previous_text=False)
        ...
        result={'audio_sha256':audio_hash,'model':'Systran/faster-whisper-small','engine_version':importlib.metadata.version('faster-whisper'),'device':'cpu','compute_type':'int8','language_hint':args.language_hint,'detected_language':info.language if args.language_hint is None else None,'transcription_language':info.language,'language_probability':info.language_probability if args.language_hint is None else None,'segments':rows,'human_reviewed':False,'status':'asr_draft_not_pronunciation_approval'}
        target.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
        vtt='WEBVTT\n\n'+''.join(f'{stamp(s["start"])} --> {stamp(s["end"])}\n{s["text"].strip()}\n\n' for s in rows)
        (folder/('captions'+suffix+'-draft.vtt')).write_text(vtt)
        print(json.dumps({'job':folder.name,'language_hint':args.language_hint,'detected_language':info.language if args.language_hint is None else None,'transcription_language':info.language,'segments':len(rows)}),flush=True)
